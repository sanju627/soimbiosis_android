...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(carController == null || heliController == null)
        {
            ...
            if (energyValue <= 100)
            {
                energyValue += Time.deltaTime * energyFillSpeed;
                energySlider.value = energyValue;
            }
        ...
        if(healing)
        {
            healingSlider.currentPercent += healSpeed * Time.deltaTime;
        }
        ...
        if(itemWorld != null)
        {
            ...
            switch (itemWorld.GetItem().itemType)
            {
                ...
                case Item.ItemType.mp7:
                    database.SendData("Item MP7", inventory.mp7Amount.ToString());
                    break;
                case Item.ItemType.mp5:
                    database.SendData("Item MP5", inventory.mp5Amount.ToString());
                    break;
                case Item.ItemType.ump45:
                    database.SendData("Item UMP", inventory.ump45Amount.ToString());
                    break;
                case Item.ItemType.tec9:
                    database.SendData("Item Tec9", inventory.tec9Amount.ToString());
                    break;
                case Item.ItemType.uzi:
                    database.SendData("Item UZI", inventory.uziAmount.ToString());
                    break;
                case Item.ItemType.ak12:
                    database.SendData("Item AK12", inventory.ak12Amount.ToString());
                    break;
                case Item.ItemType.ak74:
                    database.SendData("Item AK74", inventory.ak74Amount.ToString());
                    break;
                case Item.ItemType.g3a4:
                    database.SendData("Item G3A4", inventory.g3a4Amount.ToString());
                    break;
                case Item.ItemType.g36c:
                    database.SendData("Item G36C", inventory.g36cAmount.ToString());
                    break;
                case Item.ItemType.flamethrower:
                    database.SendData("Item Flamethrower", inventory.flamethrowerAmount.ToString());
                    break;
                case Item.ItemType.glock17:
                    database.SendData("Item Glock17", inventory.glock17Amount.ToString());
                    break;
            }
